...
cardioExercises = ['cardio', 'dancing', 'SUP']
    ...
    for n in range(0, len(exerEntries)):
        ...
        for key in exerciseDictionary:
            ...
            if exerNameMatch is not None:
                ...
                renamedEx = exerciseDictionary[matchedEx]
            # there may be sme cases in the future in which there are multiple
            # matches. In this case, we would simply expect to get the LAST
            # result back... I do not anticipate multiple matches, but will
            # adjust down the road if necessart

            # award points based on workout type
        if renamedEx in lowIntensityExercises:
            points = (exerMins / 60)
        elif renamedEx in cardioExercises:
            points = (exerMins / 30)
        elif renamedEx in crossTrainingExercises:
            points = (exerMins / 15)
        else:
            points = 0
